...
function initSkyCrane() {
    ...
    chrome.storage.local.get('logins_lock', function(storageObj) {
        console.log('logins_lock: ', storageObj);
    ...
    if (request.type == 'add_login') {
        ...
        getLoginsForSite(request.message.hostname, function(logins) {
            ...
            if (logins.length == 1) {
                ...
                if (request.message.single_login_form) {
                    offerAutologin(sender.tab.id,logins[0]);
                    return;
                }
        ...
        offerAutologin(sender.tab.id,request.login);
    }
