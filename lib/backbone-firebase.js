...
var BackboneFirebaseAdapter = function(Backbone, _) {
return (function() {

const USER_DATA_KEY = "userData";

Backbone.Firebase = function(ref, callback) {
  this._fbref = ref;
  this._children = [];
  ...
  var ready = function (snapshot) {
    ...
    _.defer(function () { callback(this) }.bind(this));
  }.bind(this);
...
_.extend(Backbone.Firebase.prototype, {
  ...
  create: function(model, cb) {
    ...
    model.toJSON({ success: (function(jsonObj) {
      this._fbref.child(USER_DATA_KEY).set(jsonObj, _.bind(function(err) {
        if (!err) {
          cb(null, model.toJSON()); // callback with plaintext attrs
        } else {
          cb("Could not create model " + model.id);
        }
      }, this));
    }).bind(this),
    error: function() { cb("Could not create model "+model.id); }});
  },
  ...
  read: function(model, cb) {
    ...
    //   return;
    // }
    // Assumes that userData is only key here
    var userData = this._children[0];
    //_.find(this._children, function(child) {
    //   return child.id == model.id;
    // });
    if (!userData) return _.defer(cb, "Record not found");
    model.parse(userData, { success: function(parsedResp) {
      cb(null, parsedResp);
    }, error: function(err) {
      cb(err);
    }});
  ...
  update: function(model, cb) {
    model.toJSON({ success: (function(jsonObj) {
      this._fbref.child(USER_DATA_KEY).update(jsonObj, function(err) {
        if (!err) {
  ...
  delete: function(model, cb) {
    this._fbref.child(USER_DATA_KEY).remove(function(err) {
      if (!err) {
        cb(null, model);
      } else {
...
Backbone.Firebase.sync = function(method, model, options, error) {
  ...
  // Note: we don't support readAll and model.id will be undefined for all reads
...
//  }
  ...
  store[method].apply(this, [model, function(err, val) {
    if (err) {
      delete model.firebase; // delete this firebase instance from model
      options.error(err);
    } else {
